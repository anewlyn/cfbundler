...
const DeliveryFrequencyDropdown = ({ className = '' }: DeliveryFrequencyDropdownProps) => {
    ...
    const onMenuKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
        ...
        const focusAt = (i: number) =>
            items[Math.max(0, Math.min(items.length - 1, i))]?.focus({ preventScroll: true } as any);
        ...
        switch (e.key) {
            ...
            case 'Spacebar': {
                ...
                const isMenuItem = !!el && el.getAttribute('role') === 'menuitemradio';
                if (isMenuItem) {
                    (el as HTMLButtonElement).click(); // only click real menu items
                }
                break;
            }
        }
